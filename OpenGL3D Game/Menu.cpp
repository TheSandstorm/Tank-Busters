...
CMenu::CMenu(std::vector<std::string> _OptVect, glm::vec2 _Pos)
{
	...
	TextShader = ShaderLoader::CreateProgram(Utility::TextShaderVert.data(), Utility::TextShaderFrag.data());
	NumMenuOptions = 0;
	CurrentOption = 0;
	...
	{
		OptionVect.push_back(std::make_shared<CTextLabel>(_OptVect[i], Utility::NormalFontString.data(), Position));
		Position.y -= 60.0f;
	}
